...
public class UserController extends HttpServlet {
	...
	// 서비스 인터페이스 타입의 변수를 선언해서 요청이 무엇이냐에 따라 하나의 변수로 여러 종류의 객체를 대입.
...
//			response.sendRedirect("/MyWeb/user/user_join.jsp"); // 절대 경로
			response.sendRedirect("user/user_join.jsp");		// 상대 경로
			break;
		...
		case "logout":
			...
			request.getSession().invalidate(); // session이 하나밖에 없기 때문에 아예 없애고 새로운 세션을 받기
