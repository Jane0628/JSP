...
<body>
	...
	<%-- <c:if test="${user == null}">
		<script>
			alert('회원만 이용 가능한 게시판입니다. 로그인 후 이용해주세요.');
                    	...
                    	<%-- 페이지 버튼 --%>
   						<c:forEach var="pageNum" begin="${pc.beginPage}" end="${pc.endPage}" >
	                        ...
	                        <a href="/MyWeb/list.board?page=${pageNum}&cpp=${pc.paging.cpp}" class="page-link"
	                           style="margin-top: 0; height: 40px; border: 1px solid #643691; ${pageNum == pc.paging.page ? 'background-color: pink; color: white' : 'color: pink'}">${pageNum}</a>
	                        </li>
               			</c:forEach>
